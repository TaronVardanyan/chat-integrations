...
import {
  BetFlowData,
  MessageResponse
} from '@hoory/protos/build/web/src/inbox-service/conversation_pb';
import { DefaultMessage } from '@hoory/constants/frontend/types';
...
type Props = {
  message: MessageResponse.AsObject | DefaultMessage;
  isInWidget?: boolean;
  widgetConfig?: WidgetConfig;
  isDisabled?: boolean;
  isLastMessage?: boolean;
  isBettingScriptsLoaded: boolean;
  onSelect: (data: string, metadata: Record<string, string>) => void;
...
function BetFlowMessage({
  ...
  const [messageData, setMessageItem] = useState<BetFlowData.AsObject>(
    message.betFlow?.data || {
      teamName: '',
      sport: '',
      sportAlias: '',
      game: '',
  ...
  const handleSelectBetOption = (optionData: SelectCallback) => {
    const newMessageData: Record<string, string> = {};
    ...
    onSelect(messageToSend, metadata);
  };
  ...
  return (
    <>
      <BettingWidget
        ...
        isBettingScriptsLoaded={isBettingScriptsLoaded}
        setBettingScriptsLoaded={setBettingScriptsLoaded}
      />
